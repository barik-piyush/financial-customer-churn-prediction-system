...
const UploadDatasetInsights = () => {
  ...
  return (
    <div style={styles.wrapper}>
      <h3 style={styles.blockTitle}>Upload Dataset</h3>

      <form onSubmit={handleBulkSubmit} style={styles.form}>
        <div style={styles.uploadArea}>
          <input type="file" id="file-upload" accept=".csv" onChange={handleFileChange} style={styles.fileInput} />
        ...
        <div style={styles.actionRow}>
          ...
          {!!sessions.length && (
            <button type="button" onClick={clearSavedSessions} style={styles.clearBtn}>
      ...
      {!sessions.length && <p style={styles.emptyText}>Upload a dataset to generate charts and save session results.</p>}

      {sessions.map((session, sessionIndex) => (
        <div key={session.id} style={styles.sessionWrap}>
          <div style={styles.sessionHeader}>
            <h4 style={styles.sessionTitle}>Dataset: {session.fileName || `Session ${sessionIndex + 1}`}</h4>
            <span style={styles.sessionMeta}>{new Date(session.createdAt).toLocaleString()}</span>
          </div>

          <div style={styles.grid}>
            <div style={styles.chartCard}><h4 style={styles.cardTitle}>Bar Chart - Prediction Counts</h4><ResponsiveContainer width="100%" height={250}><BarChart data={session.stats?.barData || []}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="label" /><YAxis allowDecimals={false} /><Tooltip /><Bar dataKey="value" fill={colors.accent} /></BarChart></ResponsiveContainer></div>
            <div style={styles.chartCard}><h4 style={styles.cardTitle}>Pie Chart - Risk Segments</h4><ResponsiveContainer width="100%" height={250}><PieChart><Pie data={session.stats?.pieData || []} dataKey="value" nameKey="name" outerRadius={84} label>{(session.stats?.pieData || []).map((entry, index) => (<Cell key={entry.name} fill={PIE_COLORS[index % PIE_COLORS.length]} />))}</Pie><Tooltip /><Legend /></PieChart></ResponsiveContainer></div>
            ...
            <div style={styles.chartCard}>
              <h4 style={styles.cardTitle}>Confusion Matrix</h4>
              {!session.stats?.hasActualLabels && <p style={styles.warningText}>No `Exited`/label column found in CSV, matrix is unavailable.</p>}
              <div style={styles.matrixGrid}>
                <div style={styles.matrixCell}>TP: {session.stats?.matrix?.TP ?? 0}</div>
                <div style={styles.matrixCell}>FP: {session.stats?.matrix?.FP ?? 0}</div>
                <div style={styles.matrixCell}>FN: {session.stats?.matrix?.FN ?? 0}</div>
                <div style={styles.matrixCell}>TN: {session.stats?.matrix?.TN ?? 0}</div>
              </div>
            </div>

            <div style={{ ...styles.chartCard, gridColumn: '1 / -1' }}>
              <h4 style={styles.cardTitle}>ROC / AUC Curve</h4>
              {!session.stats?.hasActualLabels && <p style={styles.warningText}>Upload a CSV with actual churn labels to view ROC/AUC.</p>}
          ...
          <div style={styles.resultCard}>
            <h4 style={styles.resultTitle}>Uploaded Dataset Predictions {sessionIndex === 0 ? '(Latest)' : ''}</h4>
            <table style={styles.table}><thead><tr style={{ background: colors.lightBg }}><th style={styles.tableHeader}>Customer Name</th><th style={styles.tableHeader}>Prediction</th><th style={styles.tableHeader}>Probability</th></tr></thead><tbody>{(session.results || []).map((row, idx) => (<tr key={`${session.id}-${row.name}-${idx}`} style={{ background: idx % 2 === 0 ? colors.white : colors.lightBg }}><td style={styles.tableCell}>{row.name}</td><td style={styles.tableCell}>{row.prediction}</td><td style={styles.tableCell}>{Number(row.probability).toFixed(3)}</td></tr>))}</tbody></table>
          </div>
        </div>
      ))}
    </div>
...
export default UploadDatasetInsights;
